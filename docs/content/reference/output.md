...
describes what users get. Pick a format with `-o`, or let boredapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
boredapi <command> -o table   # aligned columns for reading
boredapi <command> -o jsonl   # one JSON object per line, for piping
boredapi <command> -o json    # a single JSON array
boredapi <command> -o csv     # spreadsheet friendly
boredapi <command> -o tsv     # tab-separated
boredapi <command> -o url     # just the URL column
boredapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
boredapi <command> --fields id,title,url
```
...
```bash
boredapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
boredapi <command>            # a table, because this is a terminal
boredapi <command> | wc -l    # JSONL, because this is a pipe
```
